        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 shadow-sm flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <div className="relative w-48 h-48 flex items-center justify-center mb-2">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 120 120">
              ...
              <circle cx="60" cy="60" r={circleRadius} fill="none" stroke="#3b82f6" strokeWidth="8" strokeLinecap="round" strokeDasharray={circleCircumference} strokeDashoffset={strokeOffset} className="transition-all duration-1000 ease-out" />
            </svg>
            <div className="absolute flex flex-col items-center text-center">
              <span className="text-3xl font-bold text-white tracking-tighter">{consumed.calories}</span>
              <span className="text-neutral-500 text-xs mt-1">/ {targets.calories - 50} - {targets.calories + 50} kcal</span>
            </div>
          </div>
          <p className="text-sm text-neutral-400">You have <span className="text-white font-semibold">{Math.max(targets.calories - consumed.calories, 0)}</span> kcal remaining today.</p>
        </div>
