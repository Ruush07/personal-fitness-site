...
export default function GoalsPage() {
  ...
  useEffect(() => {
    ...
    // Standard Macro Split (2.2g Protein/kg, 25% Fat, Rest Carbs)
    const proteinGrams = Math.round(weight * 2.2); 
    const fatGrams = Math.round((finalTarget * 0.25) / 9); 
  ...
  const handleSaveGoals = async () => {
    try {
      ...
      alert("Successfully saved your flexible targets!");
    } catch (error) {
  ...
  return (
    <div className="p-6 md:p-12 max-w-5xl mx-auto min-h-screen text-neutral-100">
      <header className="mb-10">
        ...
        <p className="text-neutral-400 mt-2">Adjust your metrics to calculate a flexible, achievable range.</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" value={currentWeight} onChange={(e) => setCurrentWeight(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-3 text-white outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm text-neutral-400 mb-2">Goal Weight (kg)</label>
                <input type="number" value={goalWeight} onChange={(e) => setGoalWeight(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-3 text-white outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm text-neutral-400 mb-2">Height (cm)</label>
                <input type="number" value={heightCm} onChange={(e) => setHeightCm(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-3 text-white outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm text-neutral-400 mb-2">Age</label>
                <input type="number" value={age} onChange={(e) => setAge(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-3 text-white outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 shadow-sm">
            ...
            <div className="space-y-6">
              <div>
                ...
                <select value={activityLevel} onChange={(e) => setActivityLevel(Number(e.target.value))} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-4 text-white outline-none focus:ring-2 focus:ring-blue-500">
              ...
              <div>
                ...
                <select value={pace} onChange={(e) => setPace(Number(e.target.value))} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-4 text-white outline-none focus:ring-2 focus:ring-blue-500">
        ...
        {/* RIGHT: Live Results (With Dynamic Ranges) */}
        <div className="bg-blue-600 rounded-3xl p-6 shadow-lg flex flex-col justify-between text-white relative overflow-hidden h-full">
          <div className="relative z-10">
            <h3 className="text-blue-200 font-medium mb-1">Target Range</h3>
            <div className="flex items-baseline gap-2 mb-6">
              <span className="text-4xl md:text-5xl font-bold tracking-tighter">
                {targetCalories - 100} - {targetCalories + 100}
              </span>
              <span className="text-blue-200">kcal</span>
            </div>

            <div className="space-y-4 bg-blue-700/50 p-4 rounded-2xl border border-blue-500/30 mb-6 text-sm">
              <div className="flex justify-between items-center">
                <span className="text-blue-100">Maintenance (TDEE)</span>
                <span className="font-semibold">{tdee} kcal</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-blue-100">{isGaining ? "Daily Surplus" : "Daily Deficit"}</span>
            ...
            <h3 className="text-blue-200 font-medium mb-3 flex items-center gap-2">
              <PieChart size={16}/> Target Ranges
            </h3>
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-blue-700/50 p-2 rounded-xl border border-blue-500/30 text-center">
                <span className="block text-blue-200 text-xs mb-1">Protein</span>
                <span className="font-bold text-sm">{macros.protein - 10}-{macros.protein + 10}g</span>
              </div>
              <div className="bg-blue-700/50 p-2 rounded-xl border border-blue-500/30 text-center">
                <span className="block text-blue-200 text-xs mb-1">Carbs</span>
                <span className="font-bold text-sm">{macros.carbs - 15}-{macros.carbs + 15}g</span>
              </div>
              <div className="bg-blue-700/50 p-2 rounded-xl border border-blue-500/30 text-center">
                <span className="block text-blue-200 text-xs mb-1">Fats</span>
                <span className="font-bold text-sm">{macros.fat - 5}-{macros.fat + 5}g</span>
              </div>
            </div>
          </div>
