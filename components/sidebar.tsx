...
import { LayoutDashboard, Dumbbell, Utensils, Target, LogOut, ListChecks } from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <nav className="w-full md:w-20 h-16 md:h-screen bg-neutral-900 border-t md:border-t-0 md:border-r border-neutral-800 flex flex-row md:flex-col justify-around md:justify-between items-center px-2 md:py-8 z-[100] shrink-0 pb-safe">
      <div className="flex flex-row md:flex-col justify-around md:justify-start w-full md:w-auto gap-1 md:gap-6 items-center">
        ...
        {/* NEW ROUTINE DESIGNER LINK */}
        <Link href="/routines" className={`p-3 rounded-xl transition-all ${pathname === '/routines' ? 'text-blue-500 bg-neutral-800' : 'text-neutral-400'}`}>
          <ListChecks size={24} />
        </Link>
